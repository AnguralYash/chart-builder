...
const AdminDashboard = () => {
  ...
  const handleInputs = (e) => {
    ...
    if (e.target.name === "chartType") {
      temp.actualvalue =
        e.target.options[e.target.selectedIndex].getAttribute("actualvalue")
      if (target === "line" || target === "bar") {
        temp.indexAxis =
          e.target.options[e.target.selectedIndex].getAttribute("direction")
        temp.stacked =
          e.target.options[e.target.selectedIndex].getAttribute("stacked")
      }
    }
    // console.log(temp);
    setNewChart(temp)
  ...
  return (
    <div className="h-full flex flex-col justify-center items-center bg-dashboard">
      <button
        className="px-[25px] py-[15px] shadow-button  bg-white"
        onClick={handlemodal}
      >
        Create Project
      </button>
      <div
        className=" w-full h-full z-4 pt-1/20  bg-modalO fixed top-0 left-0"
        style={{ display: modal }}
      >
        <div className=" animate-[modal_0.5s_ease-in-out]  w-2/5 h-1/3 bg-white  shadow-modal relative top-[33%] left-[33%] flex flex-col pt-0 pb-6  pl-8 ">
          <img
            className="self-end mt-2 mr-2 mb-8 w-3%  hover:rotate-[90deg] duration-500"
            onClick={handlemodal}
          ...
          <select
            ...
            id="point_style"
            className="border border-solid border-cardinput rounded-[5px] py-[10px] px-[15px] mr-[30px] mb-[20px] outline-none"
          >
            ...
            <option
              ...
              direction="x"
              stacked="true"
              actualvalue="Vertical Stacked Bar"
            >
            ...
            <option
              ...
              direction="y"
              stacked="true"
              actualvalue="Horizontal Stacked Bar"
            >
            ...
            <option
              ...
              direction="y"
              stacked="false"
              actualvalue="Horizontal Line"
            >
              Horizontal Line
            </option>
            <option value="scatter" actualvalue="Scatter">
              Scatter
            </option>
            <option value="pie" actualvalue="Pie">
              Pie
            </option>
            <option value="doughnut" actualvalue="Doughnut">
              Doughnut
            </option>
            <option value="radar" actualvalue="Radar">
              Radar
            </option>
            <option value="polar" actualvalue="Polar">
              Polar
            </option>
          </select>
          {/* <InputControl
